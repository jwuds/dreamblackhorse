...
import { useMapDeliveryReachImage } from '@/hooks/useMapDeliveryReachImage';
import { supabase } from '@/lib/customSupabaseClient';
import { Loader2, Image as ImageIcon, CheckCircle, Search } from 'lucide-react';
import { useToast } from '@/components/ui/use-toast';

const MapDeliveryReachImageSelector = () => {
  const { image, loading, publishImage } = useMapDeliveryReachImage();
  ...
  const handleSelectImage = async (selectedImage) => {
    ...
    try {
      ...
      toast({
        title: 'Image Updated',
        description: 'Map Delivery Reach image has been successfully updated.',
      });
    } catch (err) {
      toast({
        title: 'Error',
  ...
  return (
    <div className="bg-[#1a1a1a] rounded-2xl border border-white/10 p-6">
      <h2 className="text-2xl font-bold text-white mb-6">Map Delivery Reach Image</h2>
      
      <div className="mb-8">
        <h3 className="text-lg font-medium text-white mb-4">Current Image</h3>
        <div className="aspect-[4/3] max-w-md bg-[#111] rounded-xl overflow-hidden border border-white/10 relative">
          {loading ? (
            <div className="absolute inset-0 flex items-center justify-center">
              <Loader2 className="w-8 h-8 text-gray-500 animate-spin" />
            </div>
          ) : image ? (
            <img src={image.file_path} alt={image.alt_text || "Delivery Reach"} className="w-full h-full object-cover" />
          ) : (
...
export default MapDeliveryReachImageSelector;